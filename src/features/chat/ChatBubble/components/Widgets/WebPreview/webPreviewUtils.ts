...
export const WEB_PREVIEW_LANGUAGE_TAG = 'neoclaw-web-preview';
export const WEB_PREVIEW_PLACEHOLDER_PREFIX = 'NEOCLAWWEBPREVIEW';

export function extractWebPreviewBlocks(
  content: string,
  isStreaming: boolean,
): { markdown: string; blocks: ExtractedNeoclawBlock[] } {
  const blocks: ExtractedNeoclawBlock[] = [];
  const openTag = '```' + WEB_PREVIEW_LANGUAGE_TAG;
  const closeTag = '```';
  ...
  while (true) {
    ...
    const id = `${WEB_PREVIEW_PLACEHOLDER_PREFIX}${blockIndex}_${hash}`;
...
export function parseWebPreviewJson<T>(json: string): T | null {
  try {
