...
export const VIDEO_PLAYER_LANGUAGE_TAG = 'neoclaw-video';
export const VIDEO_PLAYER_PLACEHOLDER_PREFIX = 'NEOCLAWVIDEO';
...
export function extractVideoBlocks(
  ...
  const openTag = '```' + VIDEO_PLAYER_LANGUAGE_TAG;
  const closeTag = '```';
  ...
  while (true) {
    ...
    const id = `${VIDEO_PLAYER_PLACEHOLDER_PREFIX}${blockIndex}_${hash}`;
...
export function parseVideoJson<T>(json: string): T | null {
  try {
