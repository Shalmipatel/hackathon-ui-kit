...
export const QUESTION_LANGUAGE_TAG = 'neoclaw-question';
export const QUESTION_PLACEHOLDER_PREFIX = 'NEOCLAWQUESTION';
...
export function extractQuestionBlocks(
  ...
  const openTag = '```' + QUESTION_LANGUAGE_TAG;
  const closeTag = '```';
  ...
  while (true) {
    ...
    const id = `${QUESTION_PLACEHOLDER_PREFIX}${blockIndex}_${hash}`;
...
export function parseQuestionJson<T>(json: string): T | null {
  try {
