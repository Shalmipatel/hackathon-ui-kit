...
export const CHOICES_LANGUAGE_TAG = 'neoclaw-choices';
export const CHOICES_PLACEHOLDER_PREFIX = 'NEOCLAWCHOICES';

export function extractChoicesBlocks(
  content: string,
  isStreaming: boolean,
): { markdown: string; blocks: ExtractedNeoclawBlock[] } {
  const blocks: ExtractedNeoclawBlock[] = [];
  const openTag = '```' + CHOICES_LANGUAGE_TAG;
  const closeTag = '```';
  ...
  while (true) {
    ...
    const id = `${CHOICES_PLACEHOLDER_PREFIX}${blockIndex}_${hash}`;
...
export function parseChoicesJson<T>(json: string): T | null {
  try {
